...
private:
    static cpp_int vector_to_bigint_baseq(const vector<uint32_t>& vec, uint32_t q) {
        cpp_int result = 0;
        cpp_int base = 1;
    ...
    static vector<uint32_t> bigint_to_vector_baseq(const cpp_int& value, uint32_t q, size_t expected_length) {
        cpp_int val = value;
        vector<uint32_t> vec;
    
        for (size_t i = 0; i < expected_length; ++i) {
            vec.push_back(static_cast<uint32_t>(val % q));
            val /= q;
        }
...
public:
    // Flatten PolynomialMatrix → vector<uint32_t>
    template<int Rows, int Cols>
    static vector<uint32_t> Encode(const PolynomialMatrix<Rows, Cols>& mat) {
        ...
        for (int i = 0; i < Rows; ++i) {
            for (int j = 0; j < Cols; ++j) {
                ...
                for (int k = 0; k < poly.size(); ++k) {
                    result.push_back(static_cast<uint32_t>(poly[k]));
                }
            }
        }
        return result;
    ...
    // Unflatten vector<uint32_t> → PolynomialMatrix
    template<int Rows, int Cols>
    static PolynomialMatrix<Rows, Cols> Decode(const std::vector<uint32_t>& input) {
    ...
    static vector<uint8_t> Encode_pm(const PolynomialMatrix<Rows, Cols>& input) {
        vector<uint32_t> numbers_array = PolynomialMatrixUtils::Encode(input);
    ...
    static PolynomialMatrix<Rows, Cols> Decode_pm(const std::vector<uint8_t>& input) {
        ...
        vector<uint32_t> numbers_array = PolynomialMatrixUtils::bigint_to_vector_baseq(big_number, PARAM_Q, Rows*Cols*PARAM_N);
